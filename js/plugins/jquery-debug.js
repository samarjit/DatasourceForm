...
function debugData(o_Data, s_Title, options) {
  ...
  ,
    ...
    D = [],
    i = 0 // Array to hold data, i=counter
    ,
    hasSubKeys = false,
      ...
      $.each(data, function(key, val) {
        ...
        // NULL, UNDEFINED, NUMBER or BOOLEAN
          ...
          var id = val.id,
            ...
            src = val.src,
            hrf = val.href;
          if (skip) D[i++] = k + ' ' + id ? 'id="' + id + '"' : src ? 'src="' + src + '"' : hrf ? 'href="' + hrf + '"' : cls ? 'class="' + cls + '"' : '';
...
if (!window.console.trace) window.console.trace = function(s_Title) {
  window.console.log(debugStackTrace(s_Title, {
    display: false,
    returnHTML: false,
    sort: false
  }));
...
window.console.data = function(data, title) {
  ...
  console.log(
  (title ? title + ' = ' : '') + delim[0] + ($.type(debug) === 'string' ? debug.replace(/    /g, '\t') : debug) + delim[1]);
};
...
function timer(timerName, action, options) {
  ...
  if (o.returnString) {
    ...
    if (o.padNumbers) switch (time.length) {
    case 1:
      time = "&ensp;&ensp;&ensp;" + time;
      break;
    case 2:
      time = "&ensp;&ensp;" + time;
      break;
    ...
    if (o.timePrefix || o.timeSuffix) time = o.timePrefix + time + o.timeSuffix;
  }
...
function removeUITheme(cookieName, removeCookie) {
  ...
  if (removeCookie !== false) $.cookie(cookieName || 'jquery-ui-theme', null);
};
...
function debugWindow(content, options) {
  ...
  var $W = $('<div></div>').html(content.replace(/\n/g, '<br>').replace(/  /g, ' &nbsp;')) // format as HTML
  .css(options.css);
};