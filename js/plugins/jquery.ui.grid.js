...
$.widget( "ui.grid", {
	...
	_create: function() {
		...
		if ( $.observable ) {
			...
			$([ this._toArray( this.options.source ) ]).bind( "insert remove replaceAll arrayChange propertyChange", function( event, ui ) {
				if ( event.type === "insert" ) {
					that._bindChange( ui.items );
				} else if (event.type === "replaceAll" ) {
				//	that._unbindChange( ui.oldItems );
				//	that._bindChange( ui.newItems );
				} else if (event.type === "arrayChange" ){
	...
	_bindChange: function( items ) {
		...
		$.each( items, function( index, item ) {
			...
			//$.observable(item ).bind()
			$( item ).bind( "change.grid", function() {
	...
	_newRow: function( item ) {
		...
		return this.options.rowTemplate.render(item);//data( "grid-item", item );
	},
	...
	refresh: function() {
		...
		$.each( this._toArray(), function( itemId, item ) {
			$(that._newRow( item )).appendTo( tbody ).data( "grid-item", item );
		});
	...
	_rowTemplate: function() {
		...
		this.options.rowTemplate = $.templates( template );
	},
